...
const StudentCourseContentManager = () => {
  ...
  useEffect(() => {
    ...
    const fetchChapters = async () => {
      try {
        const token = localStorage.getItem("authToken") || localStorage.getItem("token");
        const res = await axios.get(`/api/chapters/${selectedSubjectId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  useEffect(() => {
    ...
    const fetchTopics = async () => {
      try {
        const token = localStorage.getItem("authToken") || localStorage.getItem("token");
        const res = await axios.get(`/api/topics/${selectedChapterId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  useEffect(() => {
    ...
    const fetchTests = async () => {
      try {
        const token = localStorage.getItem("authToken") || localStorage.getItem("token");
        const res = await axios.get(`/api/tests/${selectedTopicId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  useEffect(() => {
    ...
    const fetchQuestions = async () => {
      try {
        const token = localStorage.getItem("authToken") || localStorage.getItem("token");
        const res = await axios.get(`/api/questions?testId=${selectedTestId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
